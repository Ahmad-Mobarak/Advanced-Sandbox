        ...
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                INSERT INTO audit_log (
                    ...
                    details, status
                ) VALUES ($1, 'sample_submitted', 'sample', $2, $3::jsonb, 'success')
                """,
                user["id"], sample_id,
                json.dumps({"file_name": file.filename, "file_size": file_size, "priority": priority})
            )
...
async def execute_agent_code(
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            "success" if result.status in ["success", "active"] else "failure"
        )
...
async def create_browser_session(
    ...
    client = get_kasm_client(
        mode=mode,
        api_url=os.getenv("KASM_URL"),
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                ...
                "success" if result.status in ["success", "safe"] else "failure"
            )
